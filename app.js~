...
io.sockets.on('connection', function (socket) {
	...
	//----------------------------PRIVATE CHAT---------------------
		...
		socket.on('send_pri_msg',function(receiverName,msg){
			...
			var collection = process.db.collection('history');
			collection.insert({'to':receiverName,'msg':msg,'from':socket.username},function(err,data){
	...
	//------------------------CRATING ROOM-----------------------
		...
		socket.on('createRoom',function(roomName,name1,name2,name3,name4){
			...
			else
			{
				...
				socket.broadcast.in(name3).emit('updatechat', 'SERVER', socket.username + ' has created the room ' + roomName + ' and invited you to join.');
				socket.broadcast.in(name4).emit('updatechat', 'SERVER', socket.username + ' has created the room ' + roomName + ' and invited you to join.');
				...
				socket.broadcast.in(name3).emit('update_crtrooms',crt_rooms);
				socket.broadcast.in(name4).emit('update_crtrooms',crt_rooms);
